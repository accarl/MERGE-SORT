...
void SortData(int counts[], int data[], double time[], int length)
{
    ...
    time[0] = chrono::duration <double, nano> (t2 - t1).count();// difference of mergesort_random
    auto diff = t2 -t1;
    ...
    time[1] = chrono::duration <double, nano> (t4 - t3).count();// difference of quicksort_random
    //auto diff1 = t2 - t1;
    ...
    time[2] = chrono::duration <double, nano> (t6 - t5).count(); //difference of mergesort_sorted
    
    auto t7 = chrono::steady_clock::now();
    QuickSort(S_tempdata, 0, length - 1, &counts[8], &counts[9]);
    auto t8 = chrono::steady_clock::now();
    time[3]= chrono::duration <double, nano> (t8 - t7).count(); //difference of quicksort_sorted
